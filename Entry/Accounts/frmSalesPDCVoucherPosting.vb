Public Class frmSalesPDCVoucherPosting
    ...
    Private Sub FillGrid(ByVal strSearch As String)
        ...
        qryMain = "EXEC SPGetbyPendingPosting @DateFrom='" & CDate(dtpDateFrom.Value).ToString("dd/MMM/yyyy") & "',@DateTo='" & CDate(dtpDateTo.Value).ToString("dd/MMM/yyyy") & "',@VType='" & cmbVType.SelectedValue.ToString() & "', @CompanyID=" & G_Compcode & ",@FinancialYearID=" & G_FinancialYearID & ""
        ...
        Try
            ...
            For Each grid As DataGridViewRow In grdRecord.Rows
                ...
                If Not IsDBNull(grid.Cells("IsPosted").Value) Then
                    ispost = Convert.ToInt32(grid.Cells("IsPosted").Value)
                    'Continue For
                    '    End If
                    ...
                    If ispost = 1 Then
                        ...
                        Else
                            isFixed = 0
                        End If
                        qryMain = "EXEC SPSaveVoucherPosting " & grid.Cells("VID").Value.ToString & ",'" & grid.Cells("VType").Value.ToString & "'," & ispost & "," & G_Compcode & "," & G_UID & "," & isFixed & ",'" & CDate(dtpPostingDate.Value).ToString("dd/MMM/yyyy") & "','" & grid.Cells("ChequeNo").Value.ToString & "','" & grid.Cells("ChequeDate").Value.ToString & "'"
    ...
    Private Sub grdAtt_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grdRecord.MouseClick
        If grdRecord.Rows.Count > 0 Then
            Dim MYID As String = grdRecord.Rows(grdRecord.CurrentCell.RowIndex).Cells(0).Value.ToString()
            'txtID.Text = MYID
            'FillForm()
            'txtID.Focus()
        End If
