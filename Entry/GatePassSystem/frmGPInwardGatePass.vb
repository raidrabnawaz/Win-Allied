...
Public Class frmGPInwardGatePass
   ...
   Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
      Try
         ...
         End If
            ...
            qryMain = "EXEC SpSaveGPInwardMain " & lblVID.Text & ", '" & txtVNo.Text & "', " & cmbSupplierCode.SelectedValue.ToString() & ", '" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "', '" & txtNarration.Text
         qryMain += "','', '" & txtDriver.Text & "', '" & txtVehicleNo.Text & "', '" & dtpTimeIn.Text & "',  " & cmbDeparment.SelectedValue.ToString() & ", " & G_Compcode & ", " & G_FinancialYearID & ", " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "'"
         MID = Convert.ToInt32(objCommon.ExecuteScalar(qryMain))
            ...
            qryMain = "EXEC SpSaveGPInwardDetail " & grid.Cells("VID").Value.ToString() & ", " & MID & ", " & grid.Cells("ProductID").Value.ToString() & "," & grid.Cells("UOMID").Value.ToString() & ",  " & grid.Cells("Quantity").Value.ToString() & ", " & grid.Cells("VRate").Value.ToString() & ", '" & grid.Cells("Remarks").Value.ToString() & "', " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "', " & If(Convert.ToBoolean(grid.Cells("IsDel").Value) = Nothing, 0, 1)
   ...
   Private Function verifySave() As String
      ...
      Next
        Dim Lock As String
        Lock = objCommon.ExecuteScalar("select dbo.fnAllowBackDate('Win-ERP','frmGPInwardGatePass','" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "'," & G_UID & "," & G_Compcode & ", " & G_FinancialYearID & ")")
   ...
   Private Sub grdAtt_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grdAtt.MouseClick
      If grdAtt.Rows.Count > 0 Then
         If grdAtt.RowCount > -1 Then
            ...
            dt = objCommon.GetDataTable("EXEC SpGetbyIDGPInwardMainDetail " & Setdouble(lblVID.Text))
   ...
   Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
      ...
      qryMain = "EXEC SpGetInvSearchLookUp 'GPIGP', @CWhere='" & cWhere & "' "
   ...
   Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click   ''pending
      MyListDic.Clear()
      rpt = (WinReportsApplication.Classess.GatePassSystemReports.SetReportName("rfGPIGP"))
      FilterVTypeList = ""
      FilterDate = ""
      ReportTitle = ""

      MyListDic.Add("@VID", lblVID.Text)
      ReportSpName = "SpRptGPInward"
