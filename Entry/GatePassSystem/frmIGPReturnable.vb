...
Public Class frmIGPReturnable
   ...
   End Sub
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        Try
         ...
         qryMain = "EXEC SpSaveGPInwardReturnableMain " & lblVID.Text & ", '" & txtVNo.Text & "', " & cmbSupplierCode.SelectedValue & ", '" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "', '" & txtNarration.Text
         qryMain += "', '" & txtDriver.Text & "', '" & txtVehicleNo.Text & "', '" & dtpTimeIn.Text & "', " & cmbDeparment.SelectedValue & ", " & G_Compcode & ", " & G_FinancialYearID & ", " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "'"
            ...
            qryMain = "EXEC SpSaveGPInWardReturnableDetail " & grid.Cells("VID").Value & ", " & MID & ", " & grid.Cells("ProductID").Value & ", " & grid.Cells("UOMID").Value & ",  " & grid.Cells("Quantity").Value & ", " & grid.Cells("VRate").Value & ", '" & grid.Cells("Remarks").Value & "', " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "', " & IIf(grid.Cells("IsDel").Value = Nothing, 0, 1)
    ...
    Private Function verifySave() As String
        ...
        End If
        Dim Lock As String
        Lock = objCommon.ExecuteScalar("select dbo.fnAllowBackDate('Win-ERP','frmIGPReturnable','" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "'," & G_UID & "," & G_Compcode & ", " & G_FinancialYearID & ")")
   ...
   End Sub
    ...
    Private Sub grdAtt_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grdAtt.MouseClick
        If grdAtt.Rows.Count > 0 Then
            If grdAtt.RowCount > -1 Then
                ...
                dt = objCommon.GetDataTable("EXEC SpGetbyIDGPInWardReturnableMainDetail " & Setdouble(lblVID.Text))
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
      Dim cWhere As String = " WHERE VDATE between ''" & Convert.ToDateTime(dtFDateFrom.Value).ToString("dd/MMM/yyyy") & "'' AND ''" & Convert.ToDateTime(dtFDateTo.Value).ToString("dd/MMM/yyyy") & "'' AND FinancialYearID = " & G_FinancialYearID
        ...
        qryMain = "EXEC SpGetInvSearchLookUp 'GPIGPReturnable', @CWhere='" & cWhere & "' "
    ...
    Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click   ''pending
        MyListDic.Clear()
      rpt = (WinReportsApplication.Classess.GatePassSystemReports.SetReportName("rfGPIGPReturnable"))
        FilterVTypeList = ""
        FilterDate = ""
        ReportTitle = ""

        MyListDic.Add("@VID", lblVID.Text)
      ReportSpName = "SpRptGPInWardReturnable"
    ...
    Private Sub txtVRate_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtPacking.KeyPress
      Dim dotLength As Integer = 1
    ...
    Private Sub txtVRate_Leave(sender As Object, e As EventArgs) Handles txtPacking.Leave
   ...
   End Sub
    ...
    Private Sub txtMachineCode_KeyUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles dtpVDate.KeyUp,
      txtPacking.KeyUp, txtNarration.KeyUp, txtSupplier.KeyUp, txtTotalQty.KeyUp, txtVCode.KeyUp, txtVName.KeyUp, txtVNo.KeyUp
        If e.KeyCode = Keys.Enter Then
            SendKeys.Send("{TAB}")
        End If 'ijaz
    End Sub 'Pending

    Private Sub txtPacking_TextChanged(sender As Object, e As EventArgs) Handles txtPacking.TextChanged
        Dim Quantity As Decimal = 0
        Dim Packicng As Decimal = 0
        If txtPacking.Text <> "" Then
            Packicng = Convert.ToDouble(Me.txtPacking.Text)
