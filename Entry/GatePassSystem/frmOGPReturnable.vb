...
Public Class frmOGPReturnable
   ...
   Private Sub frmGL_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
      ...
      objCommon.FillGPSupplierName(Me.txtSupplier, cmbSupplierCode.SelectedValue.ToString())
   ...
   Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
      ...
      ElseIf index = -1 And Me.lblDID.Text.Trim <> String.Empty Then
         ...
         Dim row As String() = New String() {lblDID.Text, lblPID.Text, Me.txtVCode.Text, Me.txtVName.Text, Me.txtRemarks.Text, Me.cmbUom.Text, Me.txtQuantity.Text, Me.txtPacking.Text, Me.cmbUom.SelectedValue, Me.txtTotalQty.Text}
   ...
   Private Sub DataGridView1_CellClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles DataGridView1.CellClick
      If DataGridView1.Rows.Count > 0 Then
         ...
         If index > -1 Then
            ...
            Me.txtRemarks.Text = DataGridView1.Rows(index).Cells("Remarks").Value.ToString()
            Me.txtQuantity.Text = DataGridView1.Rows(index).Cells("Quantity").Value.ToString()
            Me.txtPacking.Text = DataGridView1.Rows(index).Cells("VRate").Value.ToString()
            Me.txtTotalQty.Text = DataGridView1.Rows(index).Cells("Amount").Value.ToString()
         End If
      End If
   End Sub
   ...
   Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
      Try
         ...
         qryMain = "EXEC SpSaveGPOutWardReturnablemain " & lblVID.Text & ", '" & txtVNo.Text & "', " & cmbSupplierCode.SelectedValue.ToString() & ", '" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "', '" & txtNarration.Text
         qryMain += "', '" & txtDriver.Text & "', '" & txtVehicleNo.Text & "', '" & dtpTimeOut.Text & "', " & cmbDeparment.SelectedValue.ToString() & ", " & G_Compcode & ", " & G_FinancialYearID & ", " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "'"
         ...
         For Each grid As DataGridViewRow In DataGridView1.Rows
            qryMain = "EXEC SpSaveGPOutwardReturnabledetail " & grid.Cells("VID").Value.ToString() & ", " & MID & ", '" & grid.Cells("ProductID").Value.ToString() & "'," & grid.Cells("UOMID").Value.ToString() & ",  " & grid.Cells("Quantity").Value.ToString() & ", " & grid.Cells("VRate").Value.ToString() & ", '" & grid.Cells("Remarks").Value.ToString() & "', " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "', " & If(Convert.ToBoolean(grid.Cells("IsDel").Value) = Nothing, 0, 1)
   ...
   Private Function verifySave() As String
      ...
      End If
      Dim Lock As String
      Lock = objCommon.ExecuteScalar("select dbo.fnAllowBackDate('Win-ERP','frmOGPReturnable','" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "'," & G_UID & "," & G_Compcode & ", " & G_FinancialYearID & ")")
      ...
      If cmbSupplierCode.SelectedValue.ToString() = "-1" Then
   ...
   Private Sub grdAtt_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grdAtt.MouseClick
      If grdAtt.Rows.Count > 0 Then
         If grdAtt.RowCount > -1 Then
            ...
            dt = objCommon.GetDataTable("EXEC SpGetbyIDGPOutwardReturnableMainDetail " & Setdouble(lblVID.Text))
            If dt.Rows.Count > 0 Then
               lblVID.Text = dt.Rows(0)("VID").ToString()
               txtVNo.Text = dt.Rows(0)("VCode").ToString()
               dtpVDate.Value = Convert.ToDateTime(dt.Rows(0)("VDate"))
               txtNarration.Text = dt.Rows(0)("VName").ToString()
               ...
               For I As Integer = 0 To dt.Rows.Count - 1
                  Dim row As String() = New String() {dt.Rows(I)("DID").ToString(), dt.Rows(I)("ProductID").ToString(), dt.Rows(I)("ProductCode").ToString(), dt.Rows(I)("ProductName").ToString(), dt.Rows(I)("Remarks").ToString(), dt.Rows(I)("UOMName").ToString(), dt.Rows(I)("Qty").ToString(), Math.Round(Decimal.Parse(dt.Rows(I)("Packing").ToString()), 2).ToString(), dt.Rows(I)("UOMID").ToString(), Math.Round(Decimal.Parse((Convert.ToDouble(dt.Rows(I)("Qty")) * Convert.ToDouble(dt.Rows(I)("Packing"))).ToString()), 2).ToString()}
                  DataGridView1.Rows.Add(row)
               Next
            End If
         End If
   ...
   Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
      ...
      qryMain = "EXEC SpGetInvSearchLookUp 'OGPReturnable', @CWhere='" & cWhere & "' "
   ...
   Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click   ''pending
      MyListDic.Clear()
      rpt = (WinReportsApplication.Classess.GatePassSystemReports.SetReportName("rfGPOGPReturnable"))
      FilterVTypeList = ""
      FilterDate = ""
      ReportTitle = ""

      MyListDic.Add("@VID", lblVID.Text)
      ReportSpName = "SpRptGPOutwardReturnable"
   ...
   Private Sub txtVRate_KeyPress(ByVal sender As Object, ByVal e As KeyPressEventArgs) Handles txtPacking.KeyPress
      Dim dotLength As Integer = 1
      dotLength = Me.txtPacking.Text.Split("."c).Length
      If e.KeyChar <> ControlChars.Back Then
         If dotLength > 1 Then
            e.Handled = Not (Char.IsDigit(e.KeyChar))
         Else
   ...
   Private Sub txtVRate_Leave(ByVal sender As Object, ByVal e As EventArgs) Handles txtPacking.Leave
      Dim Quantity As Decimal = 0
      Dim Packicng As Decimal = 0
      If txtPacking.Text <> "" Then
         Packicng = Convert.ToDecimal(Me.txtPacking.Text)
      End If
      If txtQuantity.Text <> "" Then
         Quantity = Convert.ToDecimal(Me.txtQuantity.Text)
      End If
      Me.txtTotalQty.Text = (Quantity * Packicng).ToString()
   End Sub

   Private Sub txtInstrumentNo_KeyPress(ByVal sender As Object, ByVal e As KeyPressEventArgs) Handles txtSupplier.KeyPress
      Dim dotLength As Integer = -1
      dotLength = Me.txtSupplier.Text.Split("."c).Length
      If e.KeyChar <> ControlChars.Back Then
         If dotLength > -1 Then
            e.Handled = Not (Char.IsDigit(e.KeyChar))
         Else
   ...
   Private Sub txtPacking_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txtPacking.TextChanged
      Dim Quantity As Decimal = 0
      Dim Packicng As Decimal = 0
      If txtPacking.Text <> "" Then
         Packicng = Convert.ToDecimal(Me.txtPacking.Text)
