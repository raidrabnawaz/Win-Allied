...
Public Class frmInvFabricIGP
   ...
   Private Sub clearDet()
    ...
    Private Sub btnAdd_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnAdd.Click
        ...
        Dim PendingWeight As Double = txtpendingWeight.Text
        Dim LossWeight As Double = txtLossWeight.Text
        Dim GainWeight As Double = txtgainWeight.Text
        ...
        If Not (Setdouble(Me.txtWeight.Text) > 0) Then
            MessageBox.Show("Enter Valid Weight", "Warning", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtQuantity.Focus()
            Return
        End If
        ...
        ElseIf index = -1 And Me.lblDID.Text.Trim <> String.Empty Then
            ...
            Dim row As String() = New String() {lblDID.Text, lblPID.Text, Me.cmbUom.SelectedValue, Me.txtVCode.Text, Me.txtVName.Text, Me.cmbUom.Text, Me.txtQuantity.Text, Me.txtWeight.Text, IIf(Me.txtLossWeight.Text Is "", 0, Me.txtLossWeight.Text), IIf(Me.txtgainWeight.Text Is "", 0, Me.txtgainWeight.Text), Setdouble(Me.txtAmount.Text), Setdouble(Me.txtFreight.Text), Setdouble(Me.txtOtherCharges.Text), Setdouble(Me.txtTaxper.Text), Setdouble(Me.txtTaxAmount.Text), Setdouble(Me.txtNetAmount.Text), Me.CheckBoxIsCompleted.Checked}
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        Try
            ...
            qryMain = "EXEC SpSaveKnitFabricIGPMain " & lblVID.Text & ", '', " & lblOGPID.Text & "," & lblSupplierID.Text & ", '" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "', '" & txtNarration.Text & "', '" & txtInstrumentNo.Text
            qryMain += "','" & txtRefNo.Text & "', '" & txtDriver.Text & "','" & txtVehicleNo.Text & "','" & CDate(dtpTimeIn.Value).ToString("hh:mm tt") & "'," & cmbDepartment.SelectedValue & "," & cmbSaleOrder.SelectedValue & "," & cmbTaxGLCode.SelectedValue & ", " & G_Compcode & ", " & G_FinancialYearID & ", " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "'"
            MID = objCommon.ExecuteScalar(qryMain)
            objCommon.openConnection()
            For Each grid As DataGridViewRow In DataGridView1.Rows
                qryMain = "EXEC SpSaveKnitFabricIGPDetail " & grid.Cells("VID").Value & "," & lblOGPID.Text & ", " & MID & ", '" & grid.Cells("ProductID").Value & "', '" & grid.Cells("UOMID").Value & "',  " & grid.Cells("Quantity").Value & ",  " & grid.Cells("LossWeight").Value & ",  " & grid.Cells("GainWeight").Value & ",  " & grid.Cells("Amount").Value & ",  " & grid.Cells("Freight").Value & ",  " & grid.Cells("OtherCharges").Value & ",  " & grid.Cells("TaxPer").Value & ",  " & grid.Cells("TaxAmount").Value & ",  " & grid.Cells("NetAmount").Value & ", " & IIf(grid.Cells("IsCompleted").Value = True, 1, 0) & ", " & grid.Cells("Weight").Value & ", '-', " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "', " & IIf(grid.Cells("IsDel").Value = Nothing, 0, 1)
    ...
    Private Function verifySave() As String
        ...
        End If
        Dim Lock As String
        Lock = objCommon.ExecuteScalar("select dbo.fnAllowBackDate('Win-ERP','frmInvFabricIGP','" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "'," & G_UID & "," & G_Compcode & ", " & G_FinancialYearID & ")")
    ...
    Private Sub grdAtt_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grdAtt.MouseClick
        If grdAtt.Rows.Count > 0 Then
            If grdAtt.RowCount > -1 Then
                ...
                dt = objCommon.GetDataTable("EXEC SpGetbyIDKnitFabricIGP " & Setdouble(lblVID.Text))
                If dt.Rows.Count > 0 Then
                    ...
                    lblOGPID.Text = dt.Rows(0)("YarnOGPID")
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        qryMain = "EXEC SpGetInvSearchLookUp 'IGPFabric', @CWhere='" & cWhere & "' "
    ...
    Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click    ''pending
        MyListDic.Clear()
        rpt = (WinReportsApplication.Classess.InventoryReport.SetReportName("rfFabircIGP"))
        FilterVTypeList = ""
        FilterDate = ""
        ReportTitle = ""

        MyListDic.Add("@VID", lblVID.Text)
        ReportSpName = "SpRptKnitFabricIGP"
    ...
    Private Sub TxtOGPNo_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txtYarnOGPNo.KeyDown
        If e.KeyCode = Keys.F2 Then
            frmLookup.LookupQuery = "SELECT VID, VDate, VName, VCode, SupplierID, DepartmentID, OrderID FROM ViewKnitYarnOGP where Packing-UsedQty>0 and IsPosted=0"
    ...
    Private Sub txtYarnOGPNo_TextChanged(sender As Object, e As EventArgs) Handles txtYarnOGPNo.TextChanged
        If Not txtYarnOGPNo.Text = "" Then
            qryMain = "select Cast(Isnull(SUM(OGPD.Packing),0)as numeric(18,2)) as OGPQty, Cast(Isnull(SUM(IGPD.Packing),0)as numeric(18,2)) as IGPQty from KnitYarnOGPDetail as OGPD left outer join KnitFabricDyeingIGPMain as IGPM on 1=1 left outer join KnitFabricDyeingIGPDetail as IGPD ON IGPM.FabricOGPID=" & lblOGPID.Text & " and IGPM.vid = IGPD.MID where OGPD.MID = " & lblOGPID.Text & " "
            Dim dt As DataTable = objCommon.GetDataTable(qryMain)
            txtYarnSent.Text = dt.Rows(0)("OGPQty")
            txtFebricReceived.Text = dt.Rows(0)("IGPqty")
