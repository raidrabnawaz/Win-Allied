...
Public Class frmInvIssueNoteReturn
   ...
   Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
      Try
         ...
         qryMain = "EXEC SpSaveInvIssueReturnMain " & lblVID.Text & ",'" & txtVNo.Text & "'," & lblIssueID.Text & ",'" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "','" & txtOrderNo.Text & "','" & txtRemarks.Text & "'," & G_Compcode & "," & G_FinancialYearID & "," & G_UID & ",'" & G_IPAddress & "'," & G_UID & ",'" & G_IPAddress & "'"
         MID = objCommon.ExecuteScalar(qryMain)
            objCommon.openConnection()
            For Each grid As DataGridViewRow In DataGridView1.Rows
                qryMain = "EXEC SpSaveInvIssueReturnDetail " & grid.Cells("VID").Value & ", " & MID & ", '" & grid.Cells("ProductID").Value & "', " & grid.Cells("UomID").Value & ", " & grid.Cells("Quantity").Value & ",0," & grid.Cells("VTypePK").Value & "," & grid.Cells("StockID").Value & ",'" & grid.Cells("StockType").Value & "', " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "', " & IIf(grid.Cells("IsDel").Value = Nothing, 0, 1)
   ...
   Private Function verifySave() As String
      ...
      Next
        Dim Lock As String
        Lock = objCommon.ExecuteScalar("select dbo.fnAllowBackDate('Win-ERP','frmInvIssueNoteReturn','" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "'," & G_UID & "," & G_Compcode & ", " & G_FinancialYearID & ")")
   ...
   Private Sub grdAtt_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grdAtt.MouseClick
      If grdAtt.Rows.Count > 0 Then
         If grdAtt.RowCount > -1 Then
            ...
            dt = objCommon.GetDataTable("EXEC SpGetInvIssueReturnMainDetail " & Setdouble(lblVID.Text))
            If dt.Rows.Count > 0 Then
               lblVID.Text = dt.Rows(0)("VID")
               txtVNo.Text = dt.Rows(0)("VNo")
               dtpVDate.Value = dt.Rows(0)("VDate")
               txtOrderNo.Text = dt.Rows(0)("OrderNo")
               lblIssueID.Text = dt.Rows(0)("ISSUEID")
               txtIssueNo.Text = dt.Rows(0)("ISSUENO")
               ...
               For I As Integer = 0 To dt.Rows.Count - 1
                  Dim row As String() = New String() {dt.Rows(I)("DID"), dt.Rows(I)("ProductID"), dt.Rows(I)("UomID"), dt.Rows(I)("VCode"), dt.Rows(I)("ProductName"), dt.Rows(I)("UOM"), dt.Rows(I)("issueQty"), dt.Rows(I)("PendingQty"), 0, dt.Rows(I)("VRate"), dt.Rows(I)("Qty"), 0, 0, dt.Rows(I)("VTypePK")}
   ...
   Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        qryMain = "EXEC SpGetInvSearchLookUp 'IssueReturn', @CWhere='" & cWhere & "' "
   ...
   Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click   ''pending
      MyListDic.Clear()
      rpt = (WinReportsApplication.Classess.InventoryReport.SetReportName("rptIssueNoteReturn"))
      FilterVTypeList = ""
      FilterDate = ""
      ReportTitle = ""
      MyListDic.Add("@VID", lblVID.Text)
      ReportSpName = "SpRptInvIssueReturn"
   ...
   Private Sub TxtMDNNo_KeyDown(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txtIssueNo.KeyDown
      If e.KeyCode = Keys.F2 Then
         frmLookup.LookupQuery = "SELECT VID, VDate, VNo, VCode, ProductName FROM ViewInvIssueMainDetail WHERE (Qty - ReturnQty) > 0"
         frmLookup.LookupName = "Pending Issue Return"
         frmLookup.VID = ""
         frmLookup.VCode = ""
         frmLookup.VName = ""
         frmLookup.ShowDialog()
         If frmLookup.VID <> "" Then
            lblIssueID.Text = frmLookup.VID
            txtIssueNo.Text = frmLookup.VName
         End If
         'frmLookupCOA.Dispose()
         DataGridView1.Rows.Clear()
         txtOrderNo.Focus()
         If lblIssueID.Text IsNot "" Then
            loadDetGrid()
         End If
      End If
   End Sub
   ...
   Private Sub loadDetGrid()
      dt = objCommon.GetDataTable("Select  ProductID,UomID,VCode,ProductName,UOM,Qty,ReturnQty,UsedQty,VTypePK,DID,StockID, StockType from ViewInvIssueMainDetail where (Qty - ReturnQty) > 0 AND VID= " & Setdouble(lblIssueID.Text))
      Dim row As String()
      For Each rows As DataRow In dt.Rows
         row = New String() {0, rows("ProductID"), rows("UomID"), rows("VCode"), rows("ProductName"), rows("UOM"), rows("Qty"), Convert.ToDouble(rows("Qty")) - Convert.ToDouble(rows("ReturnQty")), rows("UsedQty"), 0, 0, rows("StockID"), rows("StockType"), rows("DID")}
         DataGridView1.Rows.Add(row)
      Next rows
   End Sub
   ...
   End Sub
    ...
    Private Sub txtMachineCode_KeyUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles dtpVDate.KeyUp,
      txtOrderNo.KeyUp, txtIssueNo.KeyUp, txtRemarks.KeyUp, txtVNo.KeyUp
