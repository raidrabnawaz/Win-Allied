...
Public Class frmSaleInvoice
   ...
   Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
      Try
         ...
         End If
            ...
            qryMain = "EXEC SpSave_SaleInvoiceMain @VID=" & lblVID.Text & ", @VNo='" & txtVNo.Text & "', @VDate='" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "',@QuoteID=0, @CustomerID=" & cmbCustomer.SelectedValue.ToString()
            qryMain += ",@AddressID=" & cmbAddress.SelectedValue.ToString() & ", @PaymentTermID=" & cmbPaymentTerm.SelectedValue.ToString() & ",@CurrencyID=" & G_CurrencyID & ", @ExchangeID=" & IIf(cmbCurrency.SelectedValue = G_CurrencyID, 0, cmbCurrency.SelectedValue) & ",@ExchangeRate=" & IIf(cmbCurrency.SelectedValue = G_CurrencyID, 1, Setdouble(txtExchangeRate.Text)) & ",@SubTotal=0,@PaymentCredit=" & txtPaymentCredit.Text & ",@Handling=0,
                @Discount=" & Setdouble(txtDiscount.Text) & ",@SalesTaxPer=" & Setdouble(txtSalesTax.Text) & ",@SalesTax=" & SaleTax & ",@SaleTaxCodeID=0,@VatPer=0,@Vat=0,@CCChargesPer=0,@CCCharges=0,@GrandTotal=" & Setdouble(lblGrandTotal.Text) & ",@Eta='--',@Instructions='" & txtRemarks.Text & "',@RefNo='" & txtRefNo.Text & "',@BankID=" & cmbBank.SelectedValue.ToString() _
            & ",@VStatus=0, @CompanyID=" & G_Compcode & ", @FinancialYearID=" & G_FinancialYearID & ", @InsertedBy =" & G_UID & ", @InsertedIp='" & G_IPAddress & "', @UpdatedBy=" & G_UID & ",@UpdatedIp='" & G_IPAddress & "'"
            MID = Convert.ToInt32(objCommon.ExecuteScalar(qryMain))
            '@VID numeric(18, 0), @MID numeric(18, 0), @SrlNo Int,@HaveAmount Int,@QuoteDID numeric(18, 0), @ProductID numeric(18, 0),
            objCommon.openConnection()
            For Each grid As DataGridViewRow In DataGridView1.Rows
                qryMain = "EXEC SpSave_SaleInvoiceDetail " & grid.Cells("VID").Value.ToString() & ", " & MID & ",0,0,0, " & grid.Cells("ProductID").Value.ToString() & ", '" & grid.Cells("VName").Value.ToString() & "','-','-'," & grid.Cells("UnitPrice").Value.ToString() & ", " & grid.Cells("Quantity").Value.ToString() & ", " & grid.Cells("Amount").Value.ToString() & ",0,'', 0,0,0,0,'', " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "'"
   ...
   End Sub
    ...
    Private Sub grdAtt_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grdAtt.MouseClick
        If grdAtt.Rows.Count > 0 Then
            If grdAtt.RowCount > -1 Then
                ...
                dt = objCommon.GetDataTable("EXEC SpGetbyIDSaleInvoiceMainDetail " & Setdouble(lblVID.Text))
                If dt.Rows.Count > 0 Then
                    ...
                    For I As Integer = 0 To dt.Rows.Count - 1
                        Dim row As String() = New String() {dt.Rows(I)("DID").ToString(), dt.Rows(I)("ProductID").ToString(), dt.Rows(I)("ProductCode").ToString(), dt.Rows(I)("ProductDesc").ToString(), dt.Rows(I)("UnitPrice").ToString(), dt.Rows(I)("Quantity").ToString(), (Setdouble(dt.Rows(I)("UnitPrice").ToString()) * Setdouble(dt.Rows(I)("Quantity").ToString())).ToString()}
                        DataGridView1.Rows.Add(row)
                    Next
                End If
            End If
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
      ...
      qryMain = "EXEC SpGetInvSearchLookUp 'SaleInvoice', '" & cWhere & "' "
   ...
   Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click   ''pending
      MyListDic.Clear()
      rpt = (WinReportsApplication.Classess.SalesReports.SetReportName("SaleInvoicePreview"))
      FilterVTypeList = ""
      FilterDate = ""
      ReportTitle = ""

      MyListDic.Add("@VID", lblVID.Text)
      ReportSpName = "SpRptSaleInvoiceMainDetail"
