...
Public Class frmSaleQuotation
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        Try
            ...
            If txtVNo.Text = "" Then
                ...
                strcode = objCommon.ExecuteScalar("select dbo.fnAccMaxCode(" & G_Compcode & "," & G_UID & "," & G_FinancialYearID & ",'SaleQuote','" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "', '" & CDate(dtpVDate.Value).ToString("MMyy") & "')")
                'strcode = Double.Parse(G_Compcode).ToString("00") & "-" & CDate(dtpVDate.Value).ToString("MMyy") & "-" & Double.Parse(strcode).ToString("0000")
                txtVNo.Text = strcode
            End If
            Dim MID As Integer = 0
            qryMain = "EXEC SpSave_SaleQuoteMain @VID=" & lblVID.Text & ", @VNo='" & txtVNo.Text & "', @VDate='" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "',  @CustomerID=" & cmbCustomer.SelectedValue.ToString()
            qryMain += ",@AddressID=" & cmbAddress.SelectedValue.ToString() & ", @PaymentTermID=" & cmbPaymentTerm.SelectedValue.ToString() & ",@CurrencyID=" & G_CurrencyID & ", @ExchangeID=" & IIf(cmbCurrency.SelectedValue = G_CurrencyID, 0, cmbCurrency.SelectedValue) & ",@ExchangeRate=" & IIf(cmbCurrency.SelectedValue = G_CurrencyID, 1, Setdouble(txtExchangeRate.Text)) & ",@SubTotal=0, @Handling=0,
            ...
            & ",  @CompanyID=" & G_Compcode & ", @InsertedBy =" & G_UID & ", @InsertedIp='" & G_IPAddress & "', @UpdatedBy=" & G_UID & ",@UpdatedIp='" & G_IPAddress & "'"
            ...
            For Each grid As DataGridViewRow In DataGridView1.Rows
                qryMain = "EXEC SpSave_SaleQuoteDetail " & grid.Cells("VID").Value.ToString() & ", " & MID & ",0,0, " & grid.Cells("ProductID").Value.ToString() & ", '" & grid.Cells("VName").Value.ToString() & "','-','-'," & grid.Cells("UnitPrice").Value.ToString() & ", " & grid.Cells("Quantity").Value.ToString() & ", " & grid.Cells("Amount").Value.ToString() & ",0,'', 0,0,0,0,'', " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "'"
    ...
    Private Sub grdAtt_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grdAtt.MouseClick
        If grdAtt.Rows.Count > 0 Then
            If grdAtt.RowCount > -1 Then
                ...
                dt = objCommon.GetDataTable("EXEC SpGetbyIDSaleQuoteMainDetail " & Setdouble(lblVID.Text))
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        qryMain = "EXEC SpGetInvSearchLookUp 'SaleQuote', '" & cWhere & "' "
    ...
    Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click   ''pending
        MyListDic.Clear()
        rpt = (WinReportsApplication.Classess.SalesReports.SetReportName("Quotation"))
        FilterVTypeList = ""
        FilterDate = ""
        ReportTitle = ""

        MyListDic.Add("@VID", lblVID.Text)
        ReportSpName = "SpRptSaleQuoteMainDetail"
