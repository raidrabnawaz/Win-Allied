...
Public Class frmSaleServiceInvoice
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        Try
            ...
            If txtVNo.Text = "" Then
                ...
                strcode = objCommon.ExecuteScalar("select dbo.fnAccMaxCode(" & G_Compcode & "," & G_UID & "," & G_FinancialYearID & ",'SaleExport','" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "', '" & CDate(dtpVDate.Value).ToString("MMyy") & "')")
                txtVNo.Text = strcode
            End If
            Dim MID As Integer = 0
            qryMain = "EXEC SpSave_SaleServiceInvoiceMain @VID=" & lblVID.Text & ", @VNo='" & txtVNo.Text & "', @VDate='" & CDate(dtpVDate.Value).ToString("dd/MMM/yyyy") & "', @QuoteID=0 ,  @CustomerID=" & cmbCustomer.SelectedValue.ToString()
            qryMain += ",@AddressID=" & cmbAddress.SelectedValue.ToString() & ", @PaymentTermID=" & cmbPaymentTerm.SelectedValue.ToString() & ",@CurrencyID=" & G_CurrencyID & ", @ExchangeID=" & IIf(cmbCurrency.SelectedValue = G_CurrencyID, 0, cmbCurrency.SelectedValue) & ",@ExchangeRate=" & IIf(cmbCurrency.SelectedValue = G_CurrencyID, 1, Setdouble(1)) & ", @PaymentCredit=" & txtPaymentCredit.Text & " ,@SubTotal=0, @Handling=0,
                @Discount=" & Setdouble(txtDiscount.Text) & ",@SalesTaxPer=" & Setdouble(txtSalesTax.Text) & ",@SalesTax=" & SaleTax & ", @VatPer=0,@Vat=0,@CCChargesPer=0,@CCCharges=0,@GrandTotal=" & Setdouble(lblGrandTotal.Text) & ",@Eta='--',@Instructions='---',@RefNo='" & txtRefNo.Text & "',@BankID=" & cmbBank.SelectedValue.ToString() _
            & ",  @CompanyID=" & G_Compcode & ", @FinancialYearID=" & G_FinancialYearID & ", @InsertedBy =" & G_UID & ", @InsertedIp='" & G_IPAddress & "', @UpdatedBy=" & G_UID & ",@UpdatedIp='" & G_IPAddress & "'"
            MID = Convert.ToInt32(objCommon.ExecuteScalar(qryMain))
            objCommon.openConnection()
            For Each grid As DataGridViewRow In DataGridView1.Rows
                qryMain = "EXEC SpSave_SaleServiceInvoiceDetail " & grid.Cells("VID").Value.ToString() & ", " & MID & ",0,0,0, " & grid.Cells("ProductID").Value.ToString() & ", '" & grid.Cells("VName").Value.ToString() & "'," & grid.Cells("UnitPrice").Value.ToString() & ",  " & grid.Cells("Amount").Value.ToString() & ",'----',0,'', 0,0,0,0,'', " & G_UID & ", '" & G_IPAddress & "', " & G_UID & ",'" & G_IPAddress & "'"
    ...
    Private Sub grdAtt_MouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles grdAtt.MouseClick
        If grdAtt.Rows.Count > 0 Then
            If grdAtt.RowCount > -1 Then
                ...
                dt = objCommon.GetDataTable("EXEC SpGetbyIDSaleServiceInvoiceMainDetail " & Setdouble(lblVID.Text))
                If dt.Rows.Count > 0 Then
                    ...
                    For I As Integer = 0 To dt.Rows.Count - 1
                        Dim row As String() = New String() {dt.Rows(I)("DID").ToString(), dt.Rows(I)("ProductID").ToString(), dt.Rows(I)("ProductCode").ToString(), dt.Rows(I)("ProductDesc").ToString(), dt.Rows(I)("UnitPrice").ToString(), (Setdouble(dt.Rows(I)("UnitPrice").ToString()) * Setdouble(dt.Rows(I)("Quantity").ToString())).ToString()}
                        DataGridView1.Rows.Add(row)
                    Next
                End If
            End If
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        qryMain = "EXEC SpGetInvSearchLookUp 'SaleService', '" & cWhere & "' "
    ...
    Private Sub cmdPrint_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdPrint.Click   ''pending
        MyListDic.Clear()
        rpt = (WinReportsApplication.Classess.SalesReports.SetReportName("SaleServiceInvoice"))
        FilterVTypeList = ""
        FilterDate = ""
        ReportTitle = ""

        MyListDic.Add("@VID", lblVID.Text)
        ReportSpName = "SpRptSaleServiceInvoiceMainDetail"
    ...
    Private Sub txtUnitPrice_KeyPress(ByVal sender As Object, ByVal e As KeyPressEventArgs) Handles txtUnitPrice.KeyPress
        Dim dotLength As Integer = 1
