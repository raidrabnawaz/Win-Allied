Public Class frmACCollection
    ...
    Private Sub FillHeader()
        ...
        'grdcol5
        '
        grdcol5.DataPropertyName = "BankCode"
        grdcol5.HeaderText = "BankCode"
        grdcol5.Name = "grdcol5"
        grdcol5.Width = 70
        '
        'grdcol4
        '
        grdcol6.DataPropertyName = "BankName"
        grdcol6.HeaderText = "BankName"
    ...
    Private Sub FillGrid(ByVal strSearch As String)
        grdRecord.DataSource = Nothing
        FillHeader()
        qryMain = " SELECT H.VID, H.VNo, H.VDate, H.PartyCode, COA.VName AS PartyName, H.ChequeNo, H.Amount, H.Remarks, H.BankCode, isnull(B.VName,'') AS BankName " & _
        " FROM dbo.tblAcCollection AS H INNER JOIN dbo.tblAcSetupCOA AS COA ON H.PartyCode = COA.VCode LEFT OUTER JOIN dbo.tblAcSetupCOA AS B ON H.BankCode = B.VCode " & _
    ...
    Private Sub FillForm()
        ...
        If Setint(txtID.Text) <= 0 Then
            If txtVno.Text <> "" Then
                qryMain = " Select top 1 VID from tblAcCollection where VNo='" & txtVno.Text & "'"
                qryMain = objCommon.ExecuteScalar(qryMain)
                txtID.Text = Setint(qryMain)
            End If
        End If
        txtID.Text = Setint(txtID.Text)
        qryMain = " SELECT H.VNo, H.VDate, H.PartyCode, COA.VName AS PartyName, H.ChequeNo, H.Amount, H.Remarks, H.BankCode, isnull(B.VName,'') AS BankName " & _
            " FROM dbo.tblAcCollection AS H INNER JOIN dbo.tblAcSetupCOA AS COA ON H.PartyCode = COA.VCode LEFT OUTER JOIN dbo.tblAcSetupCOA AS B ON H.BankCode = B.VCode WHERE H.VID=" & txtID.Text & "; "
        Dim dt As DataTable = objCommon.GetDataTable(qryMain)
        If IsNothing(dt) Then
            ClearHead(0)
            Return
      ...
      txtVName.Text = txtVName.Text.Trim.Replace("'", "")
        ...
        If txtID.Text <> "" Then 'update
            If txtVno.Text.Substring(4, 4) <> CDate(dtpDateFrom.Value).ToString("MMyy") Then
                ...
                Return
            End If
            qryMain = " update tblAcCollection set VDate='" & CDate(dtpDateFrom.Value).ToString("dd/MMM/yyyy") & "',PartyCode='" & txtCustomer.Text & "',BankCode='" & txtBank.Text & "',ChequeNo='" & txtChequeNo.Text & "',Amount=" & txtAmount.Text & ",  Remarks='" & txtVName.Text & "', UID=" & G_UID & ",Tranzdatetime=getdate(),CompCode=0 where VID = " & txtID.Text & "; "
            objCommon.ExecuteNonQuery(qryMain)
        Else
            Dim strcode As String
            strcode = objCommon.ExecuteScalar("select isnull(max(cast(left(VNo,3) as int)),0) + 1  as VCode from tblAcCollection where right(VNo,4) = '" & CDate(dtpDateFrom.Value).ToString("MMyy") & "'; ")
            strcode = Double.Parse(strcode).ToString("000") & "/" & CDate(dtpDateFrom.Value).ToString("MMyy")
            qryMain = "insert into tblAcCollection (VNo, RefNo, DocumentType, VDate, PartyCode, BankCode, ChequeNo, Amount, Remarks, IsCancel, CancelDate, IsPosted, PostedDate, FYear, UID, Tranzdatetime, CompCode ) values('" & _
    ...
    Private Sub cmdDelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        ...
        qryMain = " delete from tblAcCollection where VID = " & txtID.Text & ";"
        objCommon.ExecuteNonQuery(qryMain)
        FillGrid("")
        ClearHead(0)
