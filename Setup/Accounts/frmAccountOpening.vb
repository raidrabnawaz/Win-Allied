...
Public Class frmVoucherPosting
   ...
   Private Sub FillGrid(ByVal strSearch As String)
        ...
        If (rdbtnAccount.Checked) Then
            VType = "Account"

        ElseIf (rdbtnCustomer.Checked) Then
            VType = "Customer"
        ElseIf (rdbtnSupplier.Checked) Then
            VType = "Supplier"
        End If
      ...
      CWhere = "WHERE CompanyID=" & G_Compcode & " AND FinancialYearID IN(0, 15) and VName like ''%" & txtSearch.Text & "%''" & qry & ""

        qryMain = "EXEC GETRecordAccOpening @CWhere='" & CWhere & "', @CompanyID=" & G_Compcode & ",@FinancialYearID=" & G_FinancialYearID & ",@UserID=" & G_UID & ",@VType='" & VType & "'"
   ...
   Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
      ...
      For Each grid As DataGridViewRow In grdRecord.Rows
         ...
         End If
         If (Setdouble(myDr.Text) > 0 Or Setdouble(myCr.Text) > 0) And Setdouble(myVID.Text) = 0 Then

                qryMain = "EXEC SpSaveAccOpening '" & myVID.Text & "'," & myAccountID.Text & "," & myDr.Text & "," & myCr.Text & ",0,0,0," & G_Compcode & "," & G_FinancialYearID & "," & G_UID & ",'" & G_IPAddress & "'," & G_UID & ",'" & G_IPAddress & "','" & VType & "'"
                objCommon.ExecuteNonQuery(qryMain)
         ElseIf (Setdouble(myDr.Text) = 0 Or Setdouble(myCr.Text) = 0) And Setdouble(myVID.Text) > 0 Then
   ...
   Private Sub grdRecord_EditingControlShowing(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewEditingControlShowingEventArgs) Handles grdRecord.EditingControlShowing
      If grdRecord.CurrentCell.ColumnIndex = grdRecord.Columns("Dr").Index Then
         AddHandler CType(e.Control, TextBox).KeyPress, AddressOf TextBox_keyPress1
      ElseIf grdRecord.CurrentCell.ColumnIndex = 1 Then
         AddHandler CType(e.Control, TextBox).KeyPress, AddressOf TextBox_keyPress1
      End If
      If grdRecord.CurrentCell.ColumnIndex = grdRecord.Columns("Cr").Index Then
         AddHandler CType(e.Control, TextBox).KeyPress, AddressOf TextBox_keyPress1
      ElseIf grdRecord.CurrentCell.ColumnIndex = 1 Then
         AddHandler CType(e.Control, TextBox).KeyPress, AddressOf TextBox_keyPress1
      End If
