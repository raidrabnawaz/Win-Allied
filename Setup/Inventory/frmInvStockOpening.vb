...
Public Class frmInvStockOpening
   ...
   Private Sub frmACJV_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub FillGrid(ByVal strSearch As String)
        ...
        qryMain = "EXEC SpGet_INVStockOpening @Type='" & qry & "'"
    ...
    Private Sub grdRecord_EditingControlShowing(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewEditingControlShowingEventArgs) Handles grdRecord.EditingControlShowing
        If grdRecord.CurrentCell.ColumnIndex = grdRecord.Columns("Qty").Index Then
    ...
    Private Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        ...
        For Each grid As DataGridViewRow In grdRecord.Rows
            ...
            Dim myProductID As Label = New Label
            myProductID.Text = grid.Cells("ProductID").Value.ToString
            Dim myRate As TextBox = New TextBox
            myRate.Text = grid.Cells("VRate").Value.ToString
            Dim myQty As TextBox = New TextBox
            myQty.Text = grid.Cells("Qty").Value.ToString
            Dim myAmount As TextBox = New TextBox
            myAmount.Text = grid.Cells("Amount").Value.ToString

            'If (String.IsNullOrEmpty(myVID.Text)) Then
            '    If (Convert.ToDecimal(myQty.Text) > 0 Or Convert.ToDecimal(myAmount.Text) > 0) Then
            '        VID = 0
            '    Else
            '        Continue For
            '    End If
            'Else
            '    VID = Setdouble(myVID.Text)
            '    'VID, VNo, WarehouseID, ProductID, Qty, VRate, VValue, UsedQty, CancelQty, SerialNumber, CompanyID,  InsertedBy, InsertedDate, InsertedIp, UpdatedBy, UpdatedDate, UpdatedIp, Trailpk, TrailType
            'End If



            If Setdouble(myQty.Text) > 0 And Setdouble(myVID.Text) = 0 Then
                VID = myVID.Text
                qryMain = "EXEC SpSave_INVStockOpening '" & VID & "','',''," & myProductID.Text & "," & myRate.Text & "," & myQty.Text & "," & myAmount.Text & ",0,0,''," & G_Compcode & "," & G_UID & ",'" & G_IPAddress & "'," & G_UID & ",'" & G_IPAddress & "'"
                objCommon.ExecuteNonQuery(qryMain)
            ElseIf Setdouble(myQty.Text) = 0 And Setdouble(myVID.Text) > 0 Then
                VID = myVID.Text
                qryMain = "EXEC SpSave_INVStockOpening '" & VID & "','',''," & myProductID.Text & "," & myRate.Text & "," & myQty.Text & "," & myAmount.Text & ",0,0,''," & G_Compcode & "," & G_UID & ",'" & G_IPAddress & "'," & G_UID & ",'" & G_IPAddress & "'"
                objCommon.ExecuteNonQuery(qryMain)
            End If
    ...
    Private Sub grdRecord_CellEndEdit(sender As Object, e As DataGridViewCellEventArgs) Handles grdRecord.CellEndEdit
        Dim EQty, VRate As Double
        If e.ColumnIndex = grdRecord.Columns("Qty").Index Then
